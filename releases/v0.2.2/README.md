...
NuiPet 是基于虚拟主播鹿弈Nui形象制作的轻量桌面宠物。本版本为 Windows x64 版本，重点修复 v0.2.1 后续计划中的动作、退出、菜单视觉和动画资源问题。当前可用发布包为便携式 Windows x64 文件；安装包已归档但不可用。
...
- 保留便携式 Windows x64 文件作为本版本可用发布包。
- `NuiPet-v0.2.2-setup.exe` 已归档用于后续排查，但当前标注为不可用，请勿作为正式安装方式分发。

## 使用说明

安装包 `NuiPet-v0.2.2-setup.exe` 当前不可用，请不要运行或分发该文件。
...
- `NuiPet-v0.2.2-setup.exe` 当前不可用；请使用便携式文件启动。
